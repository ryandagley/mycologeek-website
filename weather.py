...
def list_weather_files(bucket_name, s3_client):
    try:
        ...
        response = s3_client.list_objects_v2(
            Bucket=bucket_name, Prefix='weather/')
...
def get_weather_history(bucket_name, file_name, access_key, secret_key):
    logging.info(
        f"Attempting to fetch file {file_name} from bucket {bucket_name}")
    ...
    if weather_files:
        ...
        for file in weather_files:
            data = get_weather_history(
                bucket_name, file, access_key, secret_key)
            if data:
...
if __name__ == "__main__":
    ...
    if access_key and secret_key:
        ...
        logging.error(
            "Failed to retrieve S3 credentials from Secrets Manager.")
